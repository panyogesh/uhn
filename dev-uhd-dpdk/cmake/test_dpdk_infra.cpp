...
int main(int argc, char** argv){
  if (argc < 2) {
    std::fprintf(stderr,
      ...
      "ex:    %s conf/configuration-ue.yaml -- --vdev=net_null0\n",
      argv[0], argv[0]);
    return 2;
  }
  ...
  try { cfg = flexsdr::conf::PrimaryConfig::load(yaml_path); }
  catch (const std::exception& e) { std::fprintf(stderr, "config load failed: %s\n", e.what()); return 1; }
  ...
  if (eal.init() < 0) { std::fprintf(stderr, "EAL init failed\n"); return 1; }
  ...
  switch (cfg.defaults.role) {
    ...
    case flexsdr::conf::Role::primary_gnb: {
      ...
      if (rc < 0) { std::fprintf(stderr, "primary init_resources failed: %d\n", rc); return 1; }
    ...
    case flexsdr::conf::Role::gnb: {
      ...
      app.load_config();
      rc = app.init_resources();
      if (rc < 0) { std::fprintf(stderr, "secondary init_resources failed: %d\n", rc); return 1; }
